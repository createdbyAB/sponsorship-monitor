...
`soc.py` is that test, pure and unit-tested (`python3 -m unittest test_soc`). It infers a code from the job title (`TITLE_MAP`, first match wins, with the closed codes matched first so "health and safety advisor" can never fall through to a generic rule), looks up what the code has to pay (`TSL_RATE` for shortage-list codes; the new-entrant floor for higher-skilled ones, never below £33,400) and judges the advertised salary against it — the top of the band where the advert gives one (Adzuna), otherwise the single figure stated. `soc.py` can pro-rate the floor for part-time hours, but the pipeline judges at full-time, since adverts rarely state hours. The values were checked against gov.uk on 2026-09-10.

**NHS clinical occupations** are handled separately (verified 2026-09-16). The eligible health professions — doctors, nurses and midwives, allied health professionals, pharmacists, radiographers, paramedics, psychologists, dental and eye-care practitioners, operating department practitioners, biomedical and biological scientists (`CLINICAL`) — map to their real SOC code and are eligible on the code alone, with the **pay deferred to the advert**: their salaries sit on NHS national pay scales rather than the going-rate floors `soc.py` models, and on the NHS tab the advert's own Certificate-of-Sponsorship line is the real signal anyway. The medium-skilled support roles that are closed to a first CoS — healthcare assistants, care workers, pharmacy and dental technicians (`MEDIUM_CLOSED`) — map to their code and grey out. Ambiguous roles (e.g. nursing associate) are left as `SOC ?` rather than asserted either way.

Every jobs, H&S and NHS row carries the result as `soc`, `sponsorable` and `soc_reason`, and the card shows the code as a small badge next to the employer. **Nothing is dropped.** A role whose code is closed, or whose pay is under the code's floor, is greyed out with the reason on hover, so the exclusion is visible rather than silent. A title that matches no code is *not* treated as closed: it gets a `SOC ?` badge and `check the advert`, because `TITLE_MAP` covers AB's fields, not every occupation, and reading "unmapped" as "ineligible" would grey most of a broad sweep. Only what the code can affirmatively rule out is greyed. `python monitor.py --sponsorable-only` drops exactly those ruled-out rows if you ever want a clean list; `SOC ?` and clinical rows stay, since nothing has ruled them out.

The gate runs at write time; `python monitor.py --backfill` re-judges every archived row that holds no code (untagged, `SOC ?`, or tagged under an older rule) with no network — which is how a new `TITLE_MAP` rule, like the clinical codes, reaches the archive. It never overwrites a row that already holds a code, since that verdict was judged on a band top the archive does not keep, so a change to a code's floor or rate reaches new rows only. It is idempotent, so it is safe to re-run after any change to `soc.py`.
...
**The feed is an adapter, deliberately.** `AdzunaCompanySource` implements a small `JobSource` (a `fetch(alias)` returning normalised vacancies), so a second source — Reed, or DWP's Find a job — can slot behind the same interface without touching the eligibility logic. Adzuna's own company filter is a loose keyword match, not an anchored one, so every returned advert's employer is re-verified against the company's `searchAliases`, anchored to the start of the name (the same rule that built `companies.json`); a row that does not anchor is flagged **EMPLOYER ?** and shown in a dimmed state rather than dropped or trusted. Responses are cached for 12 hours under a gitignored `.cache/`.
...
- **Environment variables.** The feed reuses the existing `ADZUNA_ID` / `ADZUNA_KEY` (see setup below). With no key, the section renders an honest empty state rather than pretending the market is quiet — it never fails the rest of the page.
- **Adding a company.** Append to `data/companies.json` following `data/README.md`: verify on the register **first**, anchored to the start of the name, list every legal entity in `searchAliases`, and set `registerCheckedOn`. A test asserts the file holds 83 entries, all `verifiedAgainstRegister`, all with non-empty aliases, so a truncated file fails the build — bump that number when you add one.
...
- **Budget.** Company watch queries every alias of every company — roughly 200 Adzuna calls a day on top of the existing ~45. `CW_CALL_CAP` bounds it, and the 12-hour cache avoids re-billing on re-runs, but a busy list may want a higher Adzuna tier; trim aliases or companies to cut back.

*(Why not scrape the 83 career sites directly? Most are JavaScript-rendered ATS front ends that break constantly and several prohibit it. The cleaner precise alternative — hitting each ATS's public JSON endpoint, as [open-jobs](https://github.com/elliottdehn/open-jobs) does across 36 of them — needs a per-company ATS+board mapping; the `JobSource` seam is there so that can be added later for the companies that warrant it.)*
