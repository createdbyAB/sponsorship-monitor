...
- **Environment variables.** ATS sourcing needs **no key** (the endpoints are public), so the 47 ATS-mapped companies work out of the box. The Adzuna fallback for the other 36 reuses the existing `ADZUNA_ID` / `ADZUNA_KEY` (see setup below). With neither an ATS map nor a key, the section renders an honest empty state rather than pretending the market is quiet — it never fails the rest of the page.
